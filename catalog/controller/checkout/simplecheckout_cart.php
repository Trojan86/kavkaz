...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController {
    ...
    public function index() {
        ...
        $this->_templateData['title__info']      = $this->language->get('title__info');
        $this->_templateData['description__info']      = $this->language->get('description__info');
        ...
        foreach ($products as $product) {
            ...
            if ($this->config->get('config_cart_weight')) {
                $weight_static = $this->weight->format($product_info['weight'], $product['weight_class_id'], $this->language->get('decimal_point'), $this->language->get('thousand_point'));
                $weight_total = $this->weight->format($product['weight'], $product['weight_class_id'], $this->language->get('decimal_point'), $this->language->get('thousand_point'));
            } else {
                $weight_static = '';
                $weight_total = '';
            }
            ...
            if ($version >= 200) {
                ...
                $this->_templateData['products'][] = array(
                    ...
                    'weight_product'      => $weight_static,
                    'weight_total'      => $weight_total,
                    'counts'     => $this->cart->countProducts()
                );
            } elseif ($version >= 156) {
                $profile_description = '';
