...
class ControllerExtensionModulePopupCart extends Controller {
	...
	public function index() {
        ...
        $data['text_btn_l'] = $this->language->get('text_btn_l');
        $data['text_btn_r'] = $this->language->get('text_btn_r');
        $data['text_info_btn'] = $this->language->get('text_info_btn');
        $data['text_information_title'] = $this->language->get('text_information_title');
        $data['text_information_text'] = $this->language->get('text_information_text');
        ...
        if($data['count'] = $this->cart->countProducts() == 0){
            $data['count']=0;
        }
        else{
		...
		foreach ($products as $product) {
			if ($product['image']) {
				$image = $this->model_tool_image->resize($product['image'], 101, 79);
			} else {
				$image = '';
			}
            ...
            if ((float)$product_info['price']) {
                $price_old = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $currency);
            } else {
                $price_old = false;
            }
	...
	private function products_related() {
		...
		if($this->cart->getProducts()) {
			...
			foreach ($products as $product_id) {
				...
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 101, 109);
				} else {
					$image = false;
				}
