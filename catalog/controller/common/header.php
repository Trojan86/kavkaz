...
class ControllerCommonHeader extends Controller {
	public function index() {
        ...
        $data['text_boss'] = $this->language->get('text_boss');
        ...
        $data['theme_url'] = 'catalog/view/theme/default';
        $data['home'] = $this->url->link('common/home');
		$data['wishlist'] = $this->url->link('account/wishlist', '', true);
		$data['logged'] = $this->customer->isLogged();
		$data['account'] = $this->url->link('account/account', '', true);
        $data['account_simple'] = $this->url->link('account/simpleedit', '', true);
		...
		foreach ($categories as $category) {
			if ($category['top']) {
				...
				$data['categories'][] = array(
					'name'     => $category['name'],
                    //Подключить вывод изображения
                    'thumb'     => $this->model_tool_image->resize($category['image'],  $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height')),
                    //
                    'icon_mob'     => $this->model_tool_image->resize($category['icon_mob'], 100, 100),
